...
! unit number has proper value and is free ?
  if (IUNIT < 10 .or. IUNIT > 999000) then
     call MESSWRT ('ERROR in FOPENG','Unit number is < 10 or > 999000 !')
     call FOPERR (IUNIT,FILNAM,STATUS,TYPE,IRECL,PRIV,IOS)
  end if
...
WRITE (*,'(/,A,/,A,I5,3(/,2A),/,A,I4,/,2A,/,A,I5,A)')     &
   ' Arguments of the call to FOPENG leading to this error:', &
   '   Unit             = ',IUNIT,                            &
   '   File name        =    ',FILNAM(1:ILFIL),               &
   '   File status      =    ',STATUS,                        &
   ...
   '   Record length    = ',IRECL,                            &
   '   Delete privilege =    ',PRIV,                          &    
   '   IOSTAT           = ',IOS, ' <-- system I/O status code'
call FATALERR ('FOPERR','FOPERR')
Return
